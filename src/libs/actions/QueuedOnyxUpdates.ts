...
function flushQueue(): Promise<void> {
    console.warn(`ddd QueuedOnyxUpdates.flushQueue()`,{queuedOnyxUpdates})
    return Onyx.update(queuedOnyxUpdates).then(() => {
        console.warn(`ddd QueuedOnyxUpdates.flushQueue() update finished`,{queuedOnyxUpdates})
        queuedOnyxUpdates = [];
    });
}

function clear(keysToPreserve: OnyxKey[]) {
    console.warn(`ddd QueuedOnyxUpdates.clear()`)
    queuedOnyxUpdates = queuedOnyxUpdates.filter((onyxUpdate) => keysToPreserve.includes(onyxUpdate.key as OnyxKey))
}
